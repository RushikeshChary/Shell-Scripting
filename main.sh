...
# Here, we use sed 1d to ignore the first row(i.e., headers). pipe to sort with key value(i.e., k4) as the 4th column values in descending order(i.e., -nr) -> pipe to head (to get top 3 values).
